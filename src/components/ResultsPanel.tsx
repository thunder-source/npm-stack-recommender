  ...
  if (!isLoading && packages.length === 0) {
    return (
      <div className="w-full max-w-4xl py-20 text-center space-y-4">
        ...
        <h3 className="text-xl font-semibold text-zinc-300">No packages found</h3>
    ...
    <div className="w-full max-w-4xl space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 p-6 bg-zinc-900/30 border border-zinc-800 rounded-2xl">
        <div className="flex items-center gap-8">
          <div className="space-y-1">
            <span className="text-[10px] uppercase tracking-wider text-zinc-500 font-bold">Total Packages</span>
            <div className="text-2xl font-bold text-zinc-100">{stats.total}</div>
          </div>
          <div className="w-px h-10 bg-zinc-800 hidden md:block" />
          <div className="space-y-1">
            <span className="text-[10px] uppercase tracking-wider text-zinc-500 font-bold">Must-have</span>
            <div className="text-2xl font-bold text-blue-400">{stats.must}</div>
          </div>
          <div className="w-px h-10 bg-zinc-800 hidden md:block" />
          <div className="space-y-1">
            <span className="text-[10px] uppercase tracking-wider text-zinc-500 font-bold">Recommended</span>
            <div className="text-2xl font-bold text-emerald-400">{stats.rec}</div>
          </div>
        </div>
      ...
      <div className="flex items-center gap-2">
        {(['all', 'must', 'rec', 'opt'] as const).map((filter) => (
          ...
          >
            {filter === 'all' ? 'All Packages' : filter === 'must' ? 'Must-have' : filter === 'rec' ? 'Recommended' : 'Optional'}
          </button>
        ))}
      </div>
